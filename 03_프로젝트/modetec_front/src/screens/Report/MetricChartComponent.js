...
const MetricChartComponent = ({ data, label, dataKey, onImageGenerated }) => {
  const chartRef = useRef(null);
  const chartInstanceRef = useRef(null);


  useEffect(() => {
    saveImageLocally(label, dataKey);
  }, [label, dataKey, data]);

  const saveImageLocally = async (label, dataKey) => {
    if (chartRef.current) {
      const metricData = data[dataKey];
      const value = parseFloat(metricData);
      const max = parseFloat(data[`max_${dataKey}`]);
